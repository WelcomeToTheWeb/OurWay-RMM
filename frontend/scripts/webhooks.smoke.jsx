...
const DEVICES = [
  {
    ...
    last_seen: "2026-08-25T11:59:00Z",
  },
  {
    id: "dev-fs-01",
...
async function fakeFetch(path, init = {}) {
  ...
  if (path === "/api/alerts/counts")
    return json({ open: 1, acked: 0, resolved: 0 });
  ...
  if (path === "/api/webhooks") {
    ...
    if (method === "POST") {
      if (!body.name || !body.url || !body.secret)
        return json({ error: "name, url and secret are required" }, 422);
      for (const c of body.categories || []) {
        if (!ALL_CATS.includes(c))
          return json({ error: `unknown category ${c}`, valid: ALL_CATS }, 422);
      }
      const ep = {
        id: nextId++,
        name: body.name,
        url: body.url,
        categories:
          body.categories && body.categories.length
            ? body.categories
            : ALL_CATS,
...
const lastCall = (re) =>
  [...fetchLog].reverse().find((c) => re.test(c.method + " " + c.path)) || null;
...
await waitUntil(
  () => text().includes("web-01") && text().includes("fileserver-01"),
  "devices view",
);
const navHrefs = [...container.querySelectorAll("nav.nav a")].map((a) =>
  a.getAttribute("href"),
);
if (
  navHrefs.slice(0, 6).join(",") !==
  "#/devices,#/alerts,#/flows,#/events,#/heal,#/webhooks"
) {
  throw new Error(
    "nav order is wrong (want devices,alerts,flows,events,heal,webhooks first): " +
      navHrefs.join(","),
  );
}
console.log(
  "ok 1: the header nav carries Webhooks as the 6th item, after Heal",
);
...
if (!pager.textContent.includes("event # 40"))
  throw new Error("the delivery cursor (event # 40) is not rendered");
if (
  !/3 consecutive/.test(pager.textContent) ||
  !/failing/.test(pager.textContent)
) {
  throw new Error("the consecutive-failure count is not rendered");
}
if (!pager.querySelector(".cat-alert"))
  throw new Error("the subscribed category badge is missing");
console.log(
  "ok 2: the endpoint list shows ops-pager — URL, alert subscription, cursor seq 40, 3 consecutive failures (failing), enabled",
);

// ---- 3. Add webhook: name/URL/secret + alert category -> POST ----------------
await act(async () => {
  btn(/add webhook/i).dispatchEvent(
    new window.MouseEvent("click", { bubbles: true }),
  );
});
await waitUntil(
  () => container.querySelector("form.webhook-form"),
...
await act(async () => {
  ...
  for (let i = 1; i < cats.length; i++)
    if (cats[i].checked) setChecked(cats[i], false);
  form.dispatchEvent(
    new window.Event("submit", { bubbles: true, cancelable: true }),
  );
});
...
const createCall = fetchLog.find(
  (c) => c.method === "POST" && c.path === "/api/webhooks",
);
if (!createCall || !createCall.body)
  throw new Error("the create POST was not sent with a body");
if (
  ...
  throw new Error(
    "the create POST body is wrong: " + JSON.stringify(createCall.body),
  );
}
const slackRow = rowFor("slack-ops");
if (!/event # 60/.test(slackRow.textContent)) {
  throw new Error(
    "the new endpoint's cursor should start at the journal tail (event # 60): " +
      slackRow.textContent,
  );
}
console.log(
  "ok 3: Add webhook posts {name, url, secret, categories:[alert]} to POST /api/webhooks; the new row appears with its cursor at the journal tail",
);
...
if (!/\/api\/webhooks\/2$/.test(patchCall.path))
  throw new Error("PATCH hit the wrong endpoint: " + patchCall.path);
if (patchCall.body.enabled !== false)
  throw new Error(
    "the PATCH body is not {enabled:false}: " + JSON.stringify(patchCall.body),
  );
await waitUntil(
  () => rowFor("slack-ops").classList.contains("off"),
  "the slack-ops row rendered disabled",
);
console.log(
  "ok 4: toggling the new endpoint off PATCHes /api/webhooks/2 with {enabled:false} and the row renders disabled",
);

// ---- 5. Deliveries: the subscribed journal, seq-colored against the cursor ---
await act(async () => {
  [...rowFor("ops-pager").querySelectorAll("button")]
    .find((b) => /deliveries/i.test(b.textContent))
    .dispatchEvent(new window.MouseEvent("click", { bubbles: true }));
});
await waitUntil(
  () => container.querySelector(".webhook-journal-meta"),
  "the deliveries panel",
);
await waitUntil(
  () => container.querySelectorAll("tr.journal-row").length === 15,
  "15 alert events (the endpoint's category)",
);
const dRows = [...container.querySelectorAll("tr.journal-row")];
if (!dRows.every((r) => r.querySelector(".cat-alert")))
  throw new Error("the journal shows non-alert events");
...
if (delivered !== 10 || pending !== 5) {
  throw new Error(
    `cursor split is wrong: delivered=${delivered}, pending=${pending} (want 10/5 at cursor 40)`,
  );
}
...
if (firstSeq !== "1" || lastSeq !== "57") {
  throw new Error(
    `journal seqs are wrong: newest=${lastSeq}, oldest=${firstSeq} (want 57/1)`,
  );
}
{
...
console.log(
  "ok 5: Deliveries shows the endpoint's subscribed events (15 alert rows, seqs 1..57 with timestamps) — 10 delivered up to the cursor, 5 pending in the backlog",
);

// ---- 6. Replay all -> confirm -> cursor reset confirmation --------------------
await act(async () => {
  btn(/replay all/i).dispatchEvent(
    new window.MouseEvent("click", { bubbles: true }),
  );
});
await waitUntil(() => btn(/yes, replay all/i), "the replay-all confirmation");
await act(async () => {
  btn(/yes, replay all/i).dispatchEvent(
    new window.MouseEvent("click", { bubbles: true }),
  );
});
await waitUntil(() => {
...
await waitUntil(
  () => text().includes("from_seq=0") && text().includes("last_seq=0"),
  "the cursor reset confirmation",
);
await waitUntil(
  () =>
    [...container.querySelectorAll("tr.journal-row")].every((r) =>
      r.querySelector(".cat-pending"),
    ),
  "the whole journal re-marked pending after the cursor reset",
);
console.log(
  "ok 6: Replay all confirms, POSTs {from_seq:0} to /api/webhooks/1/replay, shows the cursor reset (from_seq=0, last_seq=0) and the whole journal is pending again",
);

// ---- 7. delete the slack endpoint -> it disappears ----------------------------
await act(async () => {
  btn(/all webhooks/i).dispatchEvent(
    new window.MouseEvent("click", { bubbles: true }),
  );
});
await waitUntil(
  () => hookRows().length === 2,
  "back to the list with both endpoints",
);
await act(async () => {
  [...rowFor("slack-ops").querySelectorAll("button")]
    .find((b) => /delete/i.test(b.textContent))
    .dispatchEvent(new window.MouseEvent("click", { bubbles: true }));
});
await waitUntil(() => lastCall(/^DELETE /), "the DELETE call").catch(() => {});
await waitUntil(
  () => hookRows().length === 1 && !text().includes("slack-ops"),
  "slack-ops gone from the list",
);
const delCall = lastCall(/^DELETE /);
if (!/^\/api\/webhooks\/2$/.test(delCall.path))
  throw new Error("DELETE hit the wrong path: " + delCall.path);
console.log(
  "ok 7: Delete issues DELETE /api/webhooks/2 and the endpoint disappears from the list",
);
