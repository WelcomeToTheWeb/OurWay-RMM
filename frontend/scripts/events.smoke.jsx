...
const journalRows = () => [
  ...container.querySelectorAll("tbody tr.journal-row"),
];
const firstSeq = () => {
...
if (navHrefs.slice(0, 4).join(",") !== "#/devices,#/alerts,#/flows,#/events") {
  throw new Error(
    "nav order is wrong (want devices,alerts,flows,events first): " +
      navHrefs.join(","),
  );
}
...
await waitUntil(
  () => journalRows().length === 100,
  "the journal's final batch (100 rows)",
);
if (firstSeq() !== 500 || lastSeq() !== 401) {
  throw new Error(
    `expected newest-first tail page 500..401, got ${firstSeq()}..${lastSeq()}`,
  );
}
if (!text().includes("event # 401–500")) {
  throw new Error(
    "page header does not report the window 401–500: " + text().slice(0, 300),
  );
}
for (const cls of [
  "cat-alert",
...
if (badHost)
  throw new Error(
    "a row failed to resolve a device hostname: " +
      badHost.children[2].textContent,
  );
console.log(
...
if (selects.length < 2)
  throw new Error("expected category + device filter selects");
await act(async () => {
  setSelect(selects[0], "alert");
  setSelect(selects[1], "dev-web-01");
  btn(/^apply$/i).dispatchEvent(
    new window.MouseEvent("click", { bubbles: true }),
  );
});
if (
  !lastEventsCall().includes("category=alert") ||
  !lastEventsCall().includes("device=dev-web-01")
) {
  throw new Error(
    "filter request did not carry both params: " + lastEventsCall(),
  );
}
await waitUntil(
  () => journalRows().length === 125,
  "125 alert events for web-01",
);
if (firstSeq() !== 497)
  throw new Error(
    `filtered page should end at seq 497, got first=${firstSeq()}`,
  );
const notAlert = journalRows().find((r) => !r.querySelector(".pill.cat-alert"));
if (notAlert)
  throw new Error("a non-alert row slipped through the category filter");
const wrongDevice = journalRows().find(
  (r) => r.children[2].textContent.trim() !== "web-01",
);
if (wrongDevice)
  throw new Error("a row for another device slipped through the device filter");
console.log(
  "ok 4: category=alert + device=web-01 filters server-side and isolates that device's 125 alert events",
);

// ---- 4. click a row -> full envelope JSON + "Go to device" -------------------
await act(async () => {
  journalRows()[0].dispatchEvent(
    new window.MouseEvent("click", { bubbles: true }),
  );
});
await waitUntil(
  () => container.querySelector("pre.journal-json"),
  "the expanded detail pane",
);
...
if (!pane.includes('"id": 497') || !pane.includes("cpu.utilization_percent")) {
  throw new Error(
    "detail pane does not show the full envelope JSON: " + pane.slice(0, 200),
  );
}
...
console.log(
  "ok 5: clicking a row shows the full envelope JSON; 'Go to device' lands on the device view",
);
...
await waitUntil(
  () => journalRows().length === 100 && firstSeq() === 500,
  "fresh mount back at the tail",
);
await act(async () => {
  btn(/load earlier/i).dispatchEvent(
    new window.MouseEvent("click", { bubbles: true }),
  );
});
...
await waitUntil(
  () => firstSeq() === 400 && journalRows().length === 200,
  "the preceding batch (201–400)",
);
if (!btn(/jump to latest/i))
  throw new Error("'Jump to latest' is not offered while viewing history");
await act(async () => {
  btn(/jump to latest/i).dispatchEvent(
    new window.MouseEvent("click", { bubbles: true }),
  );
});
...
if (!lastEventsCall().includes("after=400")) {
  throw new Error(
    "'Jump to latest' did not re-walk to the end (after=400): " +
      lastEventsCall(),
  );
}
console.log(
  "ok 6: 'Load earlier' pages back one batch (after=200 -> 201–400); 'Jump to latest' re-walks to the tail",
);
...
await waitUntil(
  () => journalRows().some((r) => r.dataset.seq === "501"),
  "live event 501 in the table",
);
if (firstSeq() !== 501)
  throw new Error("the live event did not land at the TOP of the view");
const newJournalCalls = fetchLog
  .slice(callsBefore)
  .filter((c) => c.startsWith("GET /api/events?"));
if (newJournalCalls.length > 0) {
  throw new Error(
    "live event triggered a journal re-fetch (should be stream-only): " +
      newJournalCalls.join(" | "),
  );
}
console.log(
  "ok 7: a new event on the SSE stream appears at the top of the journal without a page refresh or re-fetch",
);
